...
df = df[['session_id', 'phone_number', '날짜', '시작시간', '통화결과', '마지막 대화', '대화명', 'STT']]
...
filter4 = df.session_id.isin(df.loc[df['대화명']=='여정엔티티_부품', 'session_id'])
df.drop(df[(filter4) & (df['대화명']=='부품')].index, inplace=True)
...
df.loc[df.session_id.isin(df.loc[(df['대화명']=='제품 소분류 (11006)')&(df.STT.str.contains('(업소용|영업용)')), 'session_id']), ['PRODUCT_GROUP', '제품', 'PRODUCT_CODE']] = ["주방가전", "냉장고", "BUSBS"]
...
df.loc[df.session_id.isin(df.loc[(df['대화명']=='제품 소분류 (11094)') & (df.STT.str.match('로보킹')), 'session_id']), ['PRODUCT_GROUP', '제품', 'PRODUCT_CODE']] = ["생활가전", "청소기", "GERBC"]
...
df.loc[df.session_id.isin(df.loc[(df['대화명']=='세부 제품 확인 (티비)_그외')&(df.STT.str.contains('일반')), 'session_id']), 'PRODUCT_CODE'] = "NOTLV"
df.loc[df.session_id.isin(df.loc[(df['대화명']=='세부 제품 확인 (티비)_그외')&(df.STT.str.contains('스탠')), 'session_id']), 'PRODUCT_CODE'] = "LSLED"
...
# 정렬
df.loc[:, ['인식INTENT','정답INTENT', '발화데이터', 'STT청취결과','STT실패사유','실패사유','개선사항','비고', '작업자','작업일']] = ""
df = df[['No', 'session_id', 'phone_number', '날짜', '시작시간', '여정', 'PRODUCT_GROUP', '제품', 'PRODUCT_CODE', '통화결과', 'call_result', 'call_tts_step', 'call_stt_step', '마지막 대화', \
    'turn', 'LAST', 'step_result', 'acc_tts_step', 'acc_stt_step', 'STEP', '대화명', 'STT', \
    '인식INTENT','정답INTENT', '발화데이터', 'STT청취결과','STT실패사유','실패사유','개선사항','비고', '작업자','작업일']]
df.sort_values(['날짜', '시작시간', 'session_id', 'turn'], inplace=True, ignore_index=True)
